...
def _load_at_risk_zip3(adi_floor = 50):
    ...
    unprotected = unprotected.drop_duplicates(subset=['_zip3']) # not sure where that duplicates coming in
...
def _load_synthetic_clinics(n=1000) -> pd.DataFrame:
    ...
    clinics = clinics[['_state','_zip5','_clinic_geo','_lat','_lng']].reset_index(drop=True)
...
def draw_status_treemap() -> None:
    ...
    """
    states = _load_states()
    protected_pct = states[states['_status']=="protected"]['census_total'].sum() / states['census_total'].sum()
    unprotected_pct = states[states['_status']=="not_protected"]['census_total'].sum() / states['census_total'].sum()
    px.treemap(states, path=["_status","_status_wp","_state"], color="_status_wp", values="census_total", height=600,
...
def draw_at_risk_vs_clinic_locations_map(states=[]) -> None:
    ...
    px.scatter_mapbox(
        ...
        height=700, zoom=3, color='_status_wp', hover_data=['_state','_zip3'],
        size='_census_total',
